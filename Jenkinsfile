    ...
    stages {
        stage('Scan Base Image') {
            steps {
                ...
                sh label: '', script: 'trivy client --remote http://54.175.205.125:8080/ --format template --template @junit.tpl -o base-image.xml centos:8'
        ...
        stage('Scan App Image') {
            steps {
                sh label: '', script: 'trivy client --exit-code 1 --severity CRITICAL,HIGH --remote http://54.175.205.125:8080/ --format template --template @junit.tpl -o app-image.xml srinathharsha/devopsinaction:1.0'
        ...
        stage('Push to DockerHub') {
            steps {
                ...
                sh label: '', script: 'docker push sudha-k/devopsinaction:1.0'
            }
        }
    }
}
